...
<Steps>
  ...
  <Step title="(Optional) Configure log access">
    Configure access for the control plane to read Agent Server deployment logs from the new namespace. See [(Optional) Configure access for reading Agent Server deployment logs](#optional-configure-access-for-reading-agent-server-deployment-logs-from-other-namespaces).
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Deploy and install the app on repositories">
    Run `helm upgrade` to apply the changes. Once pods are healthy:
...
To disable Fleet, set the following in your [`langsmith_config.yaml`](/langsmith/kubernetes#configure-your-helm-charts):
